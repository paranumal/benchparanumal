...
@kernel void BP3Global_v1(const int Nelements,
  ...
  for(int eo=0; eo<Nelements; eo+=p_Nblk; @outer(0)){
    ...
    @shared double s_Iq[p_Nblk][p_cubNq][p_cubNq][p_cubNq];	
    
    @shared double s_D[p_cubNq][p_cubNq];
    @shared double s_I[p_cubNq][p_Nq];
    
    @shared double s_Gqr[p_Nblk][p_cubNq][p_cubNq];
    @shared double s_Gqs[p_Nblk][p_cubNq][p_cubNq];

    @exclusive double r_qt;
    
    // heavy on registers (FP64, 2*3*8 for N=7)
    @exclusive double r_q[p_cubNq], r_Aq[p_cubNq];

    @exclusive int element, r_e;
	...
	for(int i=0;i<p_cubNq;++i;@inner(0)){
	  ...
	  if(r_e<Nelements){
	    ...
	    if(i<p_Nq && j<p_Nq){
	      for(int k = 0; k < p_Nq; k++) {
		//		const int base = i + j*p_Nq + element*p_Np;
...
@kernel void BP3Global_v2(const int Nelements,
			  @restrict const int  *elementList,
			  @restrict const int *localizedIds,
			  @restrict const double *ggeo,
			  @restrict const double *D,
  ...
  for(int e=0; e<Nelements; ++e; @outer(0)){
    ...
    @shared double s_Iq[p_cubNq][p_cubNq][p_cubNq];	
    
    @shared double s_D[p_cubNq][p_cubNq];
    @shared double s_I[p_cubNq][p_Nq];
    
    @shared double s_Gqr[p_cubNq][p_cubNq];
    @shared double s_Gqs[p_cubNq][p_cubNq];
...
// assume ggeo encodes built blocks
@kernel void BP3Dot_v2(const int Nelements,
		       @restrict const int  *elementList,
		       @restrict const int *localizedIds,
		       @restrict const double *ggeo,
		       @restrict const double *D,
  ...
  for(int e=0; e<Nelements; ++e; @outer(0)){
    ...
    @shared double s_q[p_Np];
    @shared volatile double s_qAq[p_Np];
    @shared volatile double s_warp[32];
